...
:: Generate 2FA code
FOR /f "delims=" %%a IN ('%current_dir%\steam-2fa-generator.exe --secret_key !steam_secret_key!') DO (
...
SET "content_args=+run_app_build %current_dir%\steam-depods\!project_name!\app_!steam_appid!.vdf"
SET "steam_cmd_args=!drm_args! !content_args! +quit"

:: Run steamcmd
%current_dir%\steam-sdk\tools\ContentBuilder\builder\steamcmd.exe +login !steam_login! !steam_password! !steam_2fa_code! !steam_cmd_args!
:: ECHO %current_dir%\steam-sdk\tools\ContentBuilder\builder\steamcmd.exe +login !steam_login! !steam_password! !steam_2fa_code! !steam_cmd_args!
